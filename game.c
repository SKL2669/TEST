...
bool isFeasible(int** level, struct Character* character, int* act, int stage, int* key) {
	...
	if (*act == 2 && (*character).position_height != STAGE_HEIGHT[stage - 1])
	{
	...
	else if (*act == 6 && (*character).position_width != STAGE_WIDTH[stage - 1])
	{
		tar_height = (*character).position_height;
		tar_width = (*character).position_width + 1;
	}
...
void player_move(int** level, int** originLevel, struct Character* player, int* act, int stage, int* key, bool* stage_pass) {
	...
	int tar_num;
	if (originLevel[tar_height][tar_width] == 2 || originLevel[tar_height][tar_width] == 5)
...
void Swi_print(int** level, int stage)
{
...
int main() {
	...
	printf("1代表通路，玩家可以移动至此\n");
	printf("2代表墙壁，每关墙壁的位置为固定，玩家无法通过\n");
